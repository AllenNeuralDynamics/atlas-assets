...
**v0.2.1 — September 24, 2026** — `HTML <https://atlas-assets.readthedocs.io/en/v0.2.1/>`__ · `GitHub <https://github.com/AllenNeuralDynamics/atlas-assets/releases/tag/v0.2.1>`__
   ...
   Specified the coordinate space ``origin`` as an ``[x, y, z]`` coordinate, measured along the space's axes from the corner of the defining template's image grid. Under ``--level full`` the validator reports ``E104`` when ``origin`` is not three numbers.
